...
目录搜索框下方提供两个小型整理按钮，可一键将当前目录的全部已点赞图片移入 `favourites/`、
未点赞图片移入 `unpopular/`（不受搜索筛选影响）。目录自动创建，移动后保留点赞状态；
目标位置已有同名文件时跳过，页面会显示移动数量及跳过或失败的结果。
Gallery 图片弹窗的点赞按钮两侧可切换上一张和下一张；“待删”按钮或快捷键 `m` 可标记、取消标记
当前图片，缩略图会同步显示“待删除”。进入当前文件夹的待删除列表后，可取消全部删除标记，
也可在确认后一次删除所有已标记图片。指定 `--cache DIR` 时，标记保存在 `DIR/deletion-marks/` 并在重启后保留；
未指定缓存目录时仅保存在当前进程内存中。图片被整理到其他目录时，其待删除标记会随图片迁移。
